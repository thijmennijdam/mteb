...
class MonoT5Reranker(RerankerWrapper):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.max_length = min(2048, self.tokenizer.model_max_length) # sometimes it's a v large number/max int
    ...
    def predict(self, input_to_rerank, **kwargs):
        ...
        if instructions is not None and instructions[0] is not None:
            # logger.info(f"Adding instructions to monot5 queries")
...
Relevant (only output one word, either "true" or "false"): [/INST] """
        ...
        logger.info(f"Using query_instruct_template of {self.query_instruct_template}")
        logger.info(f"Using template of {self.template}")
        logger.info(f"Using FollowIR-7B")
...
class MonoBERTReranker(RerankerWrapper):
    ...
    def predict(self, input_to_rerank, **kwargs):
        queries, passages, instructions = list(zip(*input_to_rerank))
        if instructions is not None and instructions[0] is not None:
            # logger.info(f"Adding instructions to MonoBERT queries")
            queries = [f"{q} {i}".strip() for i, q in zip(instructions, queries)]
...
mfollowir_7b_zho = ModelMeta(
    loader=partial(
        _loader,
        wrapper=FollowIRReranker,
        model_name_or_path="jhu-clsp/mFollowIR-7B-zho",
        fp_options="bfloat16",
    ),
    name="jhu-clsp/mFollowIR-7B-zho",
    languages=["eng_Latn"],
    open_source=True,
    revision="latest",
    release_date="2024-02-15",
...
mfollowir_7b_rus = ModelMeta(
    loader=partial(
        _loader,
        wrapper=FollowIRReranker,
        model_name_or_path="jhu-clsp/mFollowIR-7B-rus",
        fp_options="bfloat16",
    ),
    name="jhu-clsp/mFollowIR-7B-rus",
    languages=["eng_Latn"],
    open_source=True,
    revision="latest",
    release_date="2024-02-15",
...
mfollowir_7b_fas = ModelMeta(
    loader=partial(
        _loader,
        wrapper=FollowIRReranker,
        model_name_or_path="jhu-clsp/mFollowIR-7B-fas",
        fp_options="bfloat16",
    ),
    name="jhu-clsp/mFollowIR-7B-fas",
    languages=["eng_Latn"],
    open_source=True,
    revision="latest",
    release_date="2024-02-15",
...
mfollowir_7b_all = ModelMeta(
    loader=partial(
        _loader,
        wrapper=FollowIRReranker,
        model_name_or_path="jhu-clsp/mFollowIR-7B-all",
        fp_options="bfloat16",
    ),
    name="jhu-clsp/mFollowIR-7B-all",
    languages=["eng_Latn"],
    open_source=True,
    revision="latest",
    release_date="2024-02-15",
